...
``` bash
curl -s https://raw.githubusercontent.com/abdelbasset-jarray/odoo17/refs/heads/main/run.sh | sudo bash -s odoo-one 10017 20017
```
and/or run the following to set up another Odoo instance @ `localhost:11017` (default master password: `minhng.info`):

``` bash
curl -s https://raw.githubusercontent.com/abdelbasset-jarray/odoo17/refs/heads/main/run.sh | sudo bash -s odoo-two 11017 21017
```
...
* Default database password (**admin_passwd**) is odoo@odoo`, please change it @ [etc/odoo.conf#L60](/etc/odoo.conf#L60)
